"""Quick focused sweep — precompute data once, vary only risk/pyr."""
    ...
    for bar_i, ts in enumerate(eval_idx):
        ...
        if len(open_positions) < self.max_open:
            for asset, sig_df in asset_signals.items():
                ...
                risk_dist = (close_val - sl_val) if side == "long" else (sl_val - close_val)
                reward_dist = (tp_val - close_val) if side == "long" else (close_val - tp_val)
    ...
    for tp_rr in [6.0, 8.0, 10.0]:
        ...
        for risk in [0.5, 0.75, 1.0, 1.5]:
            for mpa, mt in [(2, 12), (3, 15)]:
                ...
                label = "tp%.0f/%.2f%%/pyr%d" % (tp_rr, risk, mpa)
                results.append((label, ret, result.n_trades, wr, dd, dt))

    print("{:<22s} {:>10s} {:>6s} {:>5s} {:>7s} {:>5s}".format('Config', 'Return', 'Trades', 'WR', 'MaxDD', 'Time'))
    print('-' * 60)
    results.sort(key=lambda x: x[1], reverse=True)
    for label, ret, nt, wr, dd, dt in results:
        flag = " <<<" if ret > 3200 and dd > -30 else ""
        print("{:<22s} {:>+9.1f}% {:>5d}  {:>4.1f}% {:>+6.1f}% {:>4.1f}s{}".format(label, ret, nt, wr, dd, dt, flag))
