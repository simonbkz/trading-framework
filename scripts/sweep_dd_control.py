"""Find the risk level that gives >3200% with <30% MaxDD."""
import sys
...
if __name__ == "__main__":
    ...
    header = "{:<30s} {:>8s} {:>6s} {:>5s} {:>7s} {:>7s} {:>7s}".format(
        'Config', 'Return', 'Trades', 'WR', 'MaxDD', 'AvgWin', 'AvgLos')
    print(header)
    print('-' * 78)

    configs = [
        # (risk, cd, max_trades, max_pa, label)
        # Low risk, no pyramiding
        (0.5, 2, 8, 1, '0.5%/cd2/8t'),
        (0.75, 2, 8, 1, '0.75%/cd2/8t'),
        (1.0, 2, 8, 1, '1.0%/cd2/8t'),
        (1.25, 2, 8, 1, '1.25%/cd2/8t'),
        (1.5, 2, 8, 1, '1.5%/cd2/8t'),
        (2.0, 2, 8, 1, '2.0%/cd2/8t'),
        # With pyramiding
        (0.5, 0, 12, 2, '0.5%/pyr2/12t'),
        (0.75, 0, 12, 2, '0.75%/pyr2/12t'),
        (1.0, 0, 12, 2, '1.0%/pyr2/12t'),
        (1.25, 0, 12, 2, '1.25%/pyr2/12t'),
        (1.5, 0, 12, 2, '1.5%/pyr2/12t'),
        (2.0, 0, 12, 2, '2.0%/pyr2/12t'),
        (2.5, 0, 12, 2, '2.5%/pyr2/12t'),
        # With pyr3
        (1.0, 0, 15, 3, '1.0%/pyr3/15t'),
        (1.5, 0, 15, 3, '1.5%/pyr3/15t'),
        (2.0, 0, 15, 3, '2.0%/pyr3/15t'),
        # Higher tp_rr variants
    ]

    for risk, cd, mt, mpa, label in configs:
        ...
        if len(trades) == 0 or 'pnl_pct' not in trades.columns:
            print("{:<30s} {:>+7.1f}% {:>5d}  {:>4.1f}% {:>+6.1f}% {:>+6.2f}% {:>+6.2f}%".format(
                label, 0, 0, 0, 0, 0, 0))
            continue
        ...
        print("{:<30s} {:>+7.1f}% {:>5d}  {:>4.1f}% {:>+6.1f}% {:>+6.2f}% {:>+6.2f}%".format(
            label, result.total_return_pct, result.n_trades, wr, dd, avg_win, avg_los))

    # Also test different tp_rr with best config
    print("\n=== TP R:R sweep with 1.5% risk, pyr2, 12 trades ===")
    for tp_rr in [3.0, 3.5, 4.0, 4.5, 5.0, 6.0]:
        ...
        # Need to re-compute signals with new tp_rr
        bt = PortfolioBacktester(
            market_data=market_data, regime_service=regime_svc,
            parameter_store=param_store, max_open_trades=12,
            risk_pct=1.5, trailing_stop_atr=0.0,
            trailing_activate_rr=0, cooldown_bars=0,
            min_rr=1.5, max_positions_per_asset=2,
        )
        result = bt.run(equity=10000)
        trades = result.trades_df
        if len(trades) == 0 or 'pnl_pct' not in trades.columns:
        ...
        eq = result.equity_curve
        dd = ((eq - eq.expanding().max()) / eq.expanding().max() * 100).min()
        label = "tp%.1f/1.5%%/pyr2" % tp_rr
        print("{:<30s} {:>+7.1f}% {:>5d}  {:>4.1f}% {:>+6.1f}%".format(
            label, result.total_return_pct, result.n_trades, wr, dd))
