"""Focused sweep: tp6-8 with risk 0.5-1.5%, pyr2-3."""
import sys
...
if __name__ == "__main__":
    ...
    for tp_rr in [5.0, 6.0, 7.0, 8.0, 10.0]:
        ...
        for risk in [0.5, 0.75, 1.0, 1.25, 1.5]:
            for mpa, mt in [(2, 12), (3, 15)]:
                ...
                results.append((tp_rr, risk, mpa, mt, ret, result.n_trades, wr, dd))

    # Sort by return descending, show all
    results.sort(key=lambda x: x[4], reverse=True)

    print("{:<28s} {:>9s} {:>6s} {:>5s} {:>7s}".format('Config', 'Return', 'Trades', 'WR', 'MaxDD'))
    print('-' * 60)
    for tp_rr, risk, mpa, mt, ret, nt, wr, dd in results:
        label = "tp%.0f/%.2f%%/pyr%d/%dt" % (tp_rr, risk, mpa, mt)
        flag = " ***" if ret > 3200 and dd > -30 else (" **" if ret > 3200 else (" *" if dd > -30 and ret > 500 else ""))
        print("{:<28s} {:>+8.1f}% {:>5d}  {:>4.1f}% {:>+6.1f}%{}".format(label, ret, nt, wr, dd, flag))

    # Highlight configs meeting targets
    print("\n=== CONFIGS NEAR TARGET (>3200%, <30% DD) ===")
    for tp_rr, risk, mpa, mt, ret, nt, wr, dd in results:
        if ret > 2000 and dd > -50:
            label = "tp%.0f/%.2f%%/pyr%d/%dt" % (tp_rr, risk, mpa, mt)
            print("{:<28s} {:>+8.1f}% {:>5d}  {:>4.1f}% {:>+6.1f}%".format(label, ret, nt, wr, dd))
